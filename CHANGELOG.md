...
### Changed
- **The Apple Maps tile is obscured in both TrendFitGroup assets** — `media/trendfitgroup.mp4` and `images/route-carousel.png`. The takes come from a seeded Simulator, but the routes on them are the maintainer's own, so the tile carried real street names, place names, a route line ending at a residential address, and enough coastline to place it. The treatment is a 48px Gaussian blur under a 0.22 white wash and 0.25 desaturation: no label, street grid or coastline silhouette survives it. Route outlines, the selector chips and the pace chart are untouched — none of them carries location. The Apple Maps mark stays sharp, re-drawn over the treated ground from a fixed alpha template so no original map pixel survives beneath it, which is what keeps Apple's attribution requirement satisfied while the map itself is gone. Both files keep their name, dimensions and duration, so no markup moves; the still stays palette-mode and 1206×2374, and `index.html`'s declared `width`/`height` are unchanged
- Verified that these two were the **only** site assets carrying map detail, rather than assumed. A saturation sweep over all seven videos and every image flags any large colour block, so it also flags charts, chips and the OG banner — the six other flagged assets were each confirmed by eye and by an Apple Maps wordmark match to be charts, not maps. The wordmark score separates them cleanly: 0.600 on `trendfitgroup.mp4`, 0.007 or less on everything else

### Added
- `_tools/` — the Mac-side pipeline that produces the treatment, so the assets above can be re-derived rather than reconstructed. `mapmask.py` (find the tile, frost it, restore the mark), `mask_video.py`, `mask_image.py`, `derive_attribution.py`, `check_detail.py`, and `social/` for the Instagram cuts. **Nothing in it is served**: there is no `_config.yml`, so GitHub Pages runs Jekyll with its defaults and a `_`-prefixed directory is not copied into the built site. Adding a `.nojekyll` file or a static-deploy workflow would stop that being true, and the README says so
- The tile is located per frame by a row profile of colour saturation rather than fixed coordinates, because the screen scrolls itself mid-clip and the tile sits at two different y positions. Three details are load-bearing and documented in `_tools/README.md`: the saturation threshold is low (0.12) because Apple Maps renders built-up areas in near-white, a detected run is snapped to the canonical 999×642 geometry growing from *both* edges, and every constant scales by frame width so one detector handles both the 1170-wide video and the 1206-wide still
- `check_detail.py` verifies the result independently, comparing mean image gradient inside the tile before and after: **6.02 original, 0.245 frosted worst-frame, 0 of 1285 frames above the 1.5 threshold**

### Fixed
- **Map geometry was leaking into the attribution and being re-drawn sharp.** The first pass lifted the wordmark off each tile by the rule "darker than the local blurred background", which a route line crossing that box satisfies exactly as well as glyph ink does. On 70 of the take's 1028 full-card frames a piece of route line was reproduced crisply over the frosted ground, visible under the "Maps" wordmark. Tightening the box (x=182, between the wordmark ending at 177 and the "Legal" link at 208) and the ink gate (`lum < 140`) reduced it to a stray tick but could not remove it, because the leaking pixels fall inside the wordmark's own footprint. `_tools/attribution.png` is now that same lift taken as a per-pixel median across all 1028 frames — the wordmark is in the same place in every one and survives a median, transient map detail does not — so every frame gets an identical, map-free mark
- `check_detail.py` could not see that failure and reported the take clean. Its gradient measurement **excludes** the attribution strip, on the grounds that the strip is sharp on purpose — which left the one region where the treatment deliberately re-draws detail as the one region nothing was checking. The strip now gets its own test: every ink pixel in it must fall inside the template's footprint. Run against the pre-fix master it reports 70 bad frames and exits 1, which is how the fix was confirmed rather than asserted
...
- `js/announcements.js` `VERSION` bumped to `2026-09-14-1`, and the `?v=` token on its `<script>` in `announcements.html` with it. That constant cache-busts the `data/announcements.json` fetch and its comment says to bump it whenever the data changes — which this did. The fetch also passes `cache: "no-store"`, so the JSON was not going to be served stale regardless; the bump keeps the token honest rather than fixing a live defect, and the HTML token moves too so a cached renderer cannot pin the old constant. `css/style.css` is untouched at `?v=2026-09-13-1` across all seven pages — no CSS changed
- The homepage promo video is the **v1.5 cut** (`media/App_Store_Promo-V1.5.mp4`), replacing the v1.2 one from December 2025 that predated three of the four modes. Renamed rather than dropped in place, because the filename is version-stamped and a `V1.2` name on v1.5 footage is the same staleness in a different place; the `<source>` in `index.html` moves with it. The file is a **lossless remux** — the source is already h264/yuv420p, so re-encoding a 144 kbps master would only have shed detail — with the audio track stripped (the element is `muted autoplay loop`, so it could never be heard) and `+faststart` added. 211 KB → 135 KB. This is v1.5 footage, not v2.0: it is a straight improvement on what shipped, and #34 stays open for the v2.0 re-cut
- CSS cache-bust token bumped to `?v=2026-09-13-1` across all seven pages. It had been left at `2026-09-12-1` through five commits of real rule changes — the Challenge Notifications teal wash, the `--mode-zoom` purple token and its rail, and two deleted `#tile-challenge-notifications` rules — so a returning visitor on a cached stylesheet would have got the old tile styling against the new markup
- `js/announcements.js` cache-bust bumped to the same token. It was still on `?v=2025-09-20-1` while the file itself changed on this branch, which would have served a cached reader the old renderer for the pinned `v2.0-beta` announcement — the one piece of content the beta launch depends on most. It is the only JS file carrying a token at all; `js/dashboard.js` and `js/faq.js` also changed on this branch and carry none
...
- `media/trendfitstack.mp4` re-taken: it opens on the full "Select. Trend. Improve!" animation rather than a stalled frame that jumped to the finished tagline, the trend strip scrolls to Distance so the selection happens on screen, and it now ends at the monthly stacked chart. Compare Years is `media/compare-years.mp4`'s whole subject and showing it in both made the Stack video the longer of the two (21.3 s, was 20.3 s)
- `media/compare-years.mp4` re-taken for the same trend-strip fix, and recut: it had opened on the Simulator's Home screen and the app-icon launch zoom, because the recording's own first frame was the previous take's white screen and the launch detector matched that instead (19.4 s originally)
- `media/compare-years.mp4` re-taken again, for two faults of its own. The Welcome → TrendFitStack push froze half-slid, the same cause as the Challenge take and fixed the same way — this take reads 24 months, so its first Health query is the heaviest of the seven and it was running inside the push animation. And toggling a comparison year off re-coloured the two that stayed: 2026 turned orange and 2025 green for the length of the hold, against a legend still reading blue and orange. That was an app bug, not a recording artefact, and it is fixed in the app — a year's colour is now a property of the year rather than of how many are currently shown (22.2 s)
- `media/trendfit.mp4` re-taken so the trend strip scrolls to Distance before it is selected — the chip sits past the right edge of the strip, and the first take toggled it unseen, so the chart appeared to change on its own. The strip then returns to the start and Average Heart Rate is selected in view (24.3 s, was 19.8 s)
- `media/trendfitgroup.mp4` re-taken: the screen now scrolls down to the route trend chart before the metric tour, so the difference between Pace and Speed is on screen instead of below the fold — the map fills a portrait frame and the chart sat under it. It also opens on the full "Select. Trend. Improve!" animation. Duration stays in the picker but is never selected: it reads Apple's exercise minutes, which a Simulator cannot be seeded with (22.8 s, was 19.0 s)
- `media/trendfitchallenge.mp4` re-taken: the Welcome → TrendFitChallenge transition no longer freezes half-slid — the pushed screen's first Health query used to run inside the push animation, and the recorder dropped frames through it, which is also why the screen appeared to open cropped at the top (those stalled frames were drawn before its navigation bar). The query now runs under Welcome's intro, so the push renders every frame and the screen arrives complete. Step 3's trend strip also scrolls to Distance before it is selected, the same fix the TrendFit and Stack takes got (28.2 s, was 28.3 s)
- `media/zoomablecharts.mp4` re-taken, and `#tile-zoom` brought onto the same pattern as every other video tile. It was the last recording still made the old way — a 1036×740 crop of the chart card, played by a bespoke `mouseenter` handler in an `<video class="zoom-hover-video">` sitting inside the tile's image box, with its own CSS. It opened on a chart with no indication of which screen it was, and of the four interactions the tile's own bullets name it showed two. The take is now a full-screen 1170×2544 portrait like the other six, running Welcome → TrendFit → Running + Distance and then the four in order: pinch to zoom, pan, long-press a point for its exact date and value, double-tap back to the full range (26.3 s, was 13.6 s)
- The tile now goes through `TILE_DATA` in `js/dashboard.js` like the rest, so it gets the standard `.tile-expand.has-video` panel, the shared open/close animation, keyboard and touch toggling, and a real `aria-expanded` state to announce — the bespoke handler and both `.zoom-hover-video` rules are gone. The four bullets stay as the tile's rest-state copy; the panel covers them on hover
- `#tile-zoom` also picks up the two pieces of tile chrome it had been missing: the 3px accent rail across the top and a "Learn more →" into `features.html#readable`. It is the one accent that is not a mode tint — the gestures it names belong to the chart rather than to any one screen, and they are on TrendFit and on both pannable Stack charts, so borrowing either hue would claim the tile for a mode it does not belong to. New `--mode-zoom` token on systemPurple: `#BF5AF2` on dark, which measures **3.44:1** against the lightest ground a home tile sits on — level with indigo — and `#8E3FBE` on dawn, because systemPurple's own light value is 3.77:1 on cream and `.feat-more` is a text link, not decoration. The darkened value is 5.32:1, inside the 5.16–5.46 band the other four dawn accents already sit in
- `media/challenge-notifications.mp4` re-taken, for two faults. The standings card sat motionless for **10.4 of the take's 21.3 seconds** — the Welcome → Challenge push had frozen half-slid (the same first-Health-query cause as the Challenge and Compare Years takes, and this was the last recording still making it), on top of holds that were simply too long. And the banner contradicted the screen it landed on: the card showed a red downtrend while the notification read "You're ahead of pace". That was real, not a demo-data quirk — the challenge target was a constant the seeded distance had outgrown, and the banner's words were typed into the recording script where nothing could hold them to it. The app now writes the notification from the same progress value the card draws its arrow from, and derives the target from the distance actually banked, so the two cannot disagree on any dataset — including the real Health export that will replace the synthetic one (17.1 s, was 21.3 s)
- `media/challenge-notifications.mp4` re-taken again, on review: the push it was re-taken to fix was still stalling briefly on its way from Welcome to the challenge screen. Warming the incoming screen's first Health query before the push was only half the fix — the demo script had also written its challenge into storage before pushing, so the incoming view model loaded it and ran a progress query per challenge from inside the animation, holding the slide 40% across for 1.6 s. The challenge is now written after the push lands, and the slide draws 13 frames where it drew one. The app→Home cut was rebuilt in the same pass — it had been a keystroke whose animation the recorder dropped frames through, leaving a half-shrunk app frozen for two to three seconds in half the takes; it is now a single frame, every time (17.2 s)
- `media/challenge-notifications.mp4` re-cut to the tile's aspect ratio. It was the one take still played portrait inside a landscape tile, so the whole 1170×2544 frame was letterboxed into the box and every detail on it shrank to the point of being unreadable on hover. The take is now cropped to **1170×744** — the top of the phone, which is where the banner and the standings card both are — and the tile gets a frame filled edge to edge at a size the reader can actually see. `#tile-challenge-notifications` carries a top-anchored `object-position: 50% 0%` override so the crop stays pinned to the banner if the box's ratio ever drifts
- `media/challenge-notifications.mp4` re-cut again, on review: the top third was too tight. It kept the banner and the top of the standings card, but the TrendFitChallenge screen is the take's substance and its standings card sits below that fold — so the one screen the tile is advertising was the one the crop cut away. The take is now the **top half** of the portrait recording (1170×1272) and is no longer cropped to the tile at all: it fits vertically the way it always did, and the horizontal gaps either side are filled with a blurred, darkened copy of the frame rather than with letterbox. That fill is baked into the video (rendered out at **2752×1272**) rather than layered in CSS, so the tile still holds one `<video>` and no second element to keep in sync. The 2752 is measured, not chosen for looks: `object-fit: cover` crops on whichever axis has the surplus, so a video wider than the tile only ever loses blur while a narrower one starts eating the phone — the tile is 2.155:1 at its widest (the 700px breakpoint), 1.571 at desktop and 0.882 at 400px, and 2.164 clears all three. The phone is untouched at every breakpoint above ~520px (17.9 s)
- `media/trendfitgroup.mp4` and `media/zoomablecharts.mp4` re-taken: both pushes from Welcome still froze on a single half-slid frame. Warming the incoming screen's Health query had been the fix everywhere else and was not enough here — the main thread is never blocked for more than about 0.2 s during these transitions, and the stall is the *recorder* rather than the app. `simctl io recordVideo` is a host process competing for CPU, so work started the instant the navigation path changes starves the capture and the take keeps one frame where half a second of animation should be. The app now builds the destination screen under the launch curtain before Welcome is ever revealed, holds there long enough for its first fetch to return, and holds again after each push until the slide is over. Both takes now draw the transition continuously (group 21.3 s, zoomable charts 25.6 s)
- `images/route-carousel.png` re-cut from the fixed build, together with an app fix behind it: TrendFitGroup's route strip had been opening with its first card pushed off the left edge, because the strip measured the width its edge inset is computed from from inside itself and so laid out once against a 16pt fallback. The screen hands the width down now, and the selected card sits centred from the first frame — visible in `media/trendfitgroup.mp4` as well as the still
- `#tile-challenge-notifications` drops its poster image for a six-bullet description of how the feature is actually used. The image was a generated mock-up rather than a screenshot and said so on inspection — its Notification Center pill read **"9how less"** and its first alert **"Youire ahead of pace"** — and with the walkthrough video now carrying the demonstration, a picture of the thing was doing less work than a description of it. The bullets are drawn from the shipping code rather than written around the image: the toggle is **Challenge Updates** under Settings → App Settings, the cadence picker offers daily / every 3 days / weekly / custom, `ChallengeNotificationPolicy` fires at 9:00 AM and formats each body as day count · metric · ahead-or-on-track-or-behind, and a closing note lands when the challenge ends. Same `feat-bullets` + "Learn more →" shape `#tile-zoom` already uses, pointing at `features.html#challenge`; `images/challenge-notifications.png` and the one CSS rule that centred it are deleted. The hover video is untouched
- `#tile-challenge-notifications` is tinted teal, so the last row of the homepage reads as two tiles rather than one wide panel. It shares that row with the App Store / beta CTA tile at half width each, and the CTA already carries an orange-edged wash — teal against it is the same pairing the **NOW IN PUBLIC BETA** band makes higher up the page, and for the same reason. It is a surface tint, not a mode tint: the tile has no accent rail, so it no more claims TrendFitGroup's teal than the beta band does. The band's literal values do not transfer, which is worth recording — at its own `rgba(64,200,224,.10)` over `rgba(255,140,0,.08)` this tile rendered as untinted, because the band sits on the cool blue half of the page radial where the orange stop reads as warmth, while this tile sits on the warm orange-brown half where that same stop cancelled the teal back into the ground. Resolved as a stronger teal with no warm stop (`.22` → `.07`, border `.40`): the effect is matched, not the values. Verified on both themes — like the band, it takes no dawn override, because the wash is a raw rgba over whichever ground is behind it
...
- Pinned announcement `v2.0-beta`, dated **2026-09-14** to match the deploy rather than the 2026-09-08 date it was drafted on — the same correction the sitemap needed, and for the same reason: it would have published reading six days old on the day it went live. It is the only pinned post and the renderer sorts pinned-first, so the change moves nothing. It covers TrendFitGroup, activity filtering and the LATEST badge, Compare Years, the 24- and 36-month ranges, the twelve accessibility issues, and the redesigned entry screen. It states the route bounds the app itself states — 200 most recent outdoor workouts from the last 365 days, occurrences rather than calendar days — so the site does not promise history the app will not draw
...
### Changed
- **Every walkthrough video and every feature-tile screenshot is now the maintainer's own Apple Health export**, not the synthetic dataset the pipeline was built against. All seven takes in `media/` and all six stills in `images/` were re-cut from it in one pass. What shows: real place names on the TrendFitGroup map, three genuine calendar years on Compare Years, and a stack built on Active Energy rather than Distance — the export has no cycling in it, and of the activities that do record a distance only hiking was left, landing in 5 of 24 months. Calories are carried by every activity and are the unit three sports are actually comparable in, which is what a stack claims of them
- The six tile stills are **scripted now, not captured by hand** — the same six app-repo scripts that produce the videos, stopped at a marked frame (`Tools/Demo/stills.sh` there). That closes the gap this refresh exposed: a dataset swap used to move the seven videos and leave all six stills behind, and both halves of that had already happened. `route-carousel.png` was still showing synthetic Toronto loops and a Cycling chip; `trendfitchallenge.png` showed a Workout Duration challenge, a metric a seeded Simulator cannot produce, so it predated the pipeline altogether. A still is a frame of its take rather than a separate capture, so the two cannot disagree again
- The stills' `width`/`height` attributes move to the new files' real dimensions (all six are 1206 wide, cropped from just below the status bar to just past the last content on screen). Their `object-position` declarations are gone with them: `.feat-img` is `width:100%; height:auto`, so the box already matches the file's aspect ratio and `object-fit: cover` had nothing to crop — the values had never done anything. Framing lives in the crop table in `stills.sh` instead, which is the only place it can be reproduced from
- Two tile alt strings corrected against what their pictures now show. `#tile-challenges` described "an active workout **duration** challenge" — wrong before this refresh as well as after it, since Duration reads Apple's exercise minutes and a seeded Simulator cannot hold them; it is a running distance challenge. `#tile-trendfit` claimed "distance improving over time" when the still carries two charts, distance and heart rate. Alt text is the one part of a tile nothing derives, which is why it is the part that goes stale
...
- Bumped the CSS cache-bust string to `?v=2026-09-12-1` across all seven page files, and `VERSION` in `js/faq.js` and `js/announcements.js`
